...
import com.xtremelabs.imageutils.FileEntry;
...
public class DiskDatabaseHelperTests extends ActivityInstrumentationTestCase2<MainActivity> {
	...
	private void addOrUpdateAndVerifyEntry(String url, long size, int width, int height) {
		...
		assertNotNull(dimensions.getWidth());
		assertNotNull(dimensions.getHeight());
		assertEquals(width, entry.getDimensions().getWidth().intValue());
		assertEquals(height, entry.getDimensions().getHeight().intValue());
	}
