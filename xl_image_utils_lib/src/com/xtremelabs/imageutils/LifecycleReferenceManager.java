...
class LifecycleReferenceManager {
	 ...
	 * @param scalingInfo
	 */
	public void getBitmap(Object key, RequestIdentifier requestIdentifier, ReferenceManagerListener imageManagerListener, ScalingInfo scalingInfo) {
		if (GeneralUtils.isStringBlank(requestIdentifier.getUrlOrFilename())) {
			imageManagerListener.onLoadImageFailed("Blank url");
			return;
		}
		ImageManagerCacheListener cacheListener = generateRegisteredListener(key, requestIdentifier, imageManagerListener);
		Bitmap bitmap = mImageCacher.getBitmap(requestIdentifier, cacheListener, scalingInfo);
		returnImageIfValid(imageManagerListener, bitmap);
	}

	public List<ReferenceManagerListener> removeListenersForKey(Object key) {
		return mListenerHelper.removeAllEntriesForKey(key);
	}

	public void cancelRequest(ReferenceManagerListener imageManagerListener) {
		if (Logger.logAll()) {
	...
	private ImageManagerCacheListener generateRegisteredListener(Object key, RequestIdentifier requestIdentifier, ReferenceManagerListener listener) {
	...
	private void returnImageIfValid(ReferenceManagerListener listener, Bitmap bitmap) {
		...
		public void onImageAvailable(final Bitmap bitmap, final ImageReturnedFrom returnedFrom) {
			mUiThreadHandler.post(new Runnable() {
				...
				public void run() {
					...
					ReferenceManagerListener listener = mListenerHelper.getAndRemoveListener(ImageManagerCacheListener.this);
